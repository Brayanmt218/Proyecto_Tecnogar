...
<div class="w-full py-8 px-4 sm:px-6 lg:px-8" x-data="providerTable()">
    <!-- Notificaciones -->
    @if (session('success'))
        <script>
            Swal.fire({
                ...
                <tbody class="divide-y divide-zinc-800">
                    @foreach ($providers as $provider)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ $provider->ruc }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ Str::limit($provider->company_name, 30) }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ Str::limit($provider->direction, 30) }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ $provider->phone }}</td>
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ Str::limit($provider->email, 30) }}</td>
                            <td class="px-4 py-4">
                                <span
                                    class="px-2 py-1 text-xs rounded-full {{ $provider->status ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300' }}">
                            ...
                            <td class="px-4 py-4 text-sm text-right">
                                <!-- Botón Editar -->
                                <button
                                    @click="openModal({{ $provider->id }}, '{{ addslashes($provider->ruc) }}', '{{ addslashes($provider->company_name) }}', '{{ addslashes($provider->direction) }}', '{{ addslashes($provider->phone) }}', '{{ addslashes($provider->email) }}', {{ $provider->status }})"
                                ...
                                <form id="delete-form-{{ $provider->id }}"
                                    action="{{ route('admin.proveedor.destroy', $provider->id) }}" method="POST"
                                    class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                        ...
                        <div class="px-8 py-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" x-model="currentRazonSocial" name="company_name"
                                        class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white"
                                        readonly>
                                </div>
                                <!-- Campo Dirección -->
                                <div>
                                    ...
                                    <input type="text" x-model="currentDireccion" name="direction"
                                        class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white"
                                        readonly>
                                </div>
                                <!-- Campo Teléfono -->
                                <div>
                                    ...
                                    <input type="text" x-model="currentTelefono" name="phone"
...
<script>
    ...
    function providerTable() {
        return {
            ...
            openModal(id, ruc, company_name, direction, phone, email, status) {
                this.currentId = id;
                this.currentRuc = ruc;
                this.currentRazonSocial = company_name;
                this.currentDireccion = direction;
                this.currentTelefono = phone;
