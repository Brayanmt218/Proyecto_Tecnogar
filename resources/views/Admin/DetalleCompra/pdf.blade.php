...
<body>
    <div class="report-header">
        <div class="header-info">
            <h1>Reporte de Detalle Compra</h1>
    ...
    <div class="report-info">
        ...
        <div>
            <strong>Total de Detalle_compra:</strong> {{ count($detail_buys) }}
        </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="40%">Fecha-Detalle_Compra</th>
                <th width="20%">Cantidad</th>
                <th width="10%">Precio U.</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($detail_buys as $index => $detail_buy)
                <tr>
                    <td class="text-center">{{ $index + 1 }}</td>
                    <td>{{ $detail_buy->product->name }}</td>
                    <td>{{ $detail_buy->buy->date }}</td>
                    <td>{{ $detail_buy->stock }}</td>
                    <td>{{ $detail_buy->price_unit }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
